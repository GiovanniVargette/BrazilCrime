get_sinesp_data <- function(state = 'all', typology = 'all', year = 'all',
  ...
  # Construir caminho completo para o arquivo ufs.csv
  ufs_path <- system.file("data-raw", "ufs.csv", package = "BrazilCrime")

  if (ufs_path == "") {
    stop("Arquivo ufs.csv não encontrado.")
  }

  ufs <- read.csv(ufs_path)
  ...
  if (relative_values == T & granularity == 'month') {
    ...
    pop_path <- system.file("data-raw", "pop_projetada_mensal_dia_15.xlsx", package = "BrazilCrime")
  ...
  if (relative_values == T & granularity == 'year') {
    ...
    pop_path <- system.file("data-raw", "pop_projetada_anual.xlsx", package = "BrazilCrime")
    pop <- readxl::read_excel(pop_path) |>
      dplyr::select(-cod)
