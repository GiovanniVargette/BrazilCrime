...
#'
#' @param dados Data.frame contendo as colunas da série: evento, uf, municipio, data, e uma coluna com os valores numéricos.
#' @param ts_col Nome da coluna (string) com os dados da série temporal (ex: "total_vitima").
#' @param freq Frequência temporal: "monthly" ou "yearly". Padrão é "monthly".
#' @param h Horizonte da previsão. Padrão: 12.
#' @param log Lógico. Se TRUE, aplica transformação logarítmica. Padrão: FALSE.
#' @param level Nível de confiança do intervalo de previsão. Padrão: 95.
#'
#' @return Lista com modelo ajustado, previsão e gráfico ggplot2.
#' @export
br_crime_predict <- function(dados, ts_col, freq = "monthly", h = 12, log = FALSE, level = 95) {
  if (missing(dados) || missing(ts_col)) {
    stop("Você deve fornecer o data.frame 'dados' e o nome da coluna da série temporal ('ts_col').")
  }
  ...
  p <- ggplot2::ggplot(df_plot, ggplot2::aes(x = date)) +
    ...
    ggplot2::geom_line(ggplot2::aes(y = forecast, color = "Previsão"), linetype = "dashed", linewidth = 1.2, na.rm = TRUE) +
    ggplot2::geom_ribbon(ggplot2::aes(ymin = lower, ymax = upper, fill = conf_label), alpha = 0.25, na.rm = TRUE) +
    ggplot2::labs(
      title = paste0("Previsão para ", tipo_crime),
    ...
    ggplot2::scale_color_manual(values = c("Observado" = "#1B9E77", "Previsão" = "#D95F02")) +
